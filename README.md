...
`python cli.py` を実行すると、12の定型記録テンプレートから番号を選び、閾値や年度などのパラメータを入力するだけで、DuckDBに対してSQLが実行され、結果が表形式で表示されます。
...
### テンプレート11〜12 (Statcastバットトラッキング・守備発展テンプレート) の実装メモ
...
mlb-record-finder/
  ...
  query/
    templates.py         12の記録検索テンプレート (SQL/関数)
  i18n/
    ja.py / en.py         UI文言辞書
  cli.py                対話式CLI本体
  requirements.txt
